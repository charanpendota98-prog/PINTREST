...
class ReelMaker:
    ...
    def make(self, image_path: str, hook: str, title: str, price_label: str,
             out_path: str | Path, source: str = "") -> str:
        ...
        for i in range(n_prod):
            t = i / n_prod
            writer.send(self._frame_product(photo, title, price_label, t).tobytes())
        for i in range(n_cta):
            writer.send(self._frame_cta(source, i / n_cta).tobytes())
        writer.close()
        log.info("Reel rendered: %s (%d frames)", out_path.name, n_hook + n_prod + n_cta)
        return str(out_path)
    ...
    def _frame_product(self, photo: Image.Image, title: str, price: str, t: float) -> Image.Image:
