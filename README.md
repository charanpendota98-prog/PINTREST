...
| 3 | **Video / Idea pins** — up to 9× the reach of static pins | Video stops the scroll [2](https://affiliatemarketingforsuccess.com/affiliate-marketing/affiliate-marketing-on-pinterest/) | `video.auto_reel` — builds a reel out of the WHOLE product gallery (model-wearing shot first via `pick_hero`, a different ken-burns move per photo, slanted **"JUST ₹299"** sticker + real ★/rating chip; emoji are stripped because the bundled font cannot draw them □) when the page has no video; real product videos downloaded when they exist. Photos are requested at FULL size (Amazon/Flipkart/Meesho CDN rules), PIL-verified before use, and the reel opens with the product blurred behind the headline |
| 4 | **Hook in first 1–3 seconds** ("Wait for the price 👀") | Curiosity = watch time = distribution | `bot/growth.py` HOOKS in reel intro + captions — **store-true**: a Meesho post can never carry an "Amazon" hook (`hook_for(source=…)` filters foreign store names) |
